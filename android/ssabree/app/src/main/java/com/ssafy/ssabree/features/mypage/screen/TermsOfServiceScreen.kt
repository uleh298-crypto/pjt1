...
fun TermsOfServiceScreen(onBackClick: () -> Unit) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("서비스 이용약관", fontWeight = FontWeight.Bold, fontSize = 18.sp) },
                navigationIcon = {
        ...
        Column(
            modifier = Modifier
                .padding(padding)
                .fillMaxSize()
                // ✨ CommunityRulesScreen.kt에 있는 scrollbar를 자동으로 가져와 씁니다!
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(24.dp)) {
                    Text(text = TERMS_CONTENT, fontSize = 14.sp, lineHeight = 24.sp, color = MaterialTheme.colorScheme.onSurface)
                    Spacer(modifier = Modifier.height(24.dp))
                    Text(text = "이 약관은 2024년 5월 30일에 개정되었습니다.", fontSize = 13.sp, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
...
private const val TERMS_CONTENT = """제1조(목적) 싸브리타임 서비스 이용약관은 삼성 SW·AI 교육센터와 함께하는 프로젝트를 통해 제작된 싸브리타임 서비스(이하 "서비스"라 합니다)의 이용과 관련하여 서비스 운영진(이하 "운영진"이라 합니다)과 싸용자 간의 권리, 의무 및 책임 사항 등을 규정함을 목적으로 합니다.

제2조(정의)
1. 이 약관에서 사용하는 용어의 정의는 다음과 같습니다.
- "서비스"란, 회사가 제공하는 모든 서비스 및 기능을 말합니다.
- "싸용자"란, 이 약관에 따라 서비스를 이용하는 회원 및 비회원을 말합니다.
- "회원"이란, 서비스에 회원등록을 하고 서비스를 이용하는 자를 말합니다. 
- "비회원"이란, 서비스에 회원등록을 하지 않고 서비스를 이용하는 자를 말합니다. 
- "게시물"이란, 서비스에 게재된 문자, 사진, 영상, 첨부파일, 광고 등을 말합니다. 
- "커뮤니티"란, 게시물을 게시할 수 있는 공간을 말합니다. "이용 기록"이란, 이용자가 서비스를 이용하면서 직접 생성한 시간표, 친구, 학점 정보 등을 말합니다. 
...
- "본인 인증"이란, MatterMost 등을 이용한 본인 확인 절차를 말합니다. 제1항에서 정의되지 않은 이 약관 내 용어의 의미는 일반적인 이용관행에 의합니다.
2. 제 1항에서 정의한 용어 외에 이 약관에서 사용하는 용어의 의미는 관련법 및 개별 서비스가 제공하는 기능의 설명에서 정하는 바에 따릅니다.
...
- 회사는 약관을 개정할 경우, 적용 일자 및 개정 사유를 명시하여 현행약관과 함께 개정약관 적용 일자 7일 전부터 "공지사항"을 통해 공지합니다. 
- 다만, 개정 내용이 회원의 권리 및 의무에 중대한 영향을 미치는 경우에는 적용 일자 30일 전부터 회원의 연락처 또는 서비스 내부 알림 수단으로 개별 공지합니다. 
- 회원은 개정 약관에 동의하지 않을 경우, 제7조(서비스 이용계약의 종료)에 따른 회원 탈퇴 방법으로 거부 의사를 표시할 수 있습니다. 단, 회사가 약관 개정 시 "개정 약관의 적용 일자까지 회원이 거부 의사를 표시하지 아니할 경우 약관의 개정에 동의한 것으로 간주한다"는 내용을 고지하였음에도 불구하고 회원이 약관 개정에 대한 거부 의사를 표시하지 아니하면, 회사는 적용 일자부로 개정 약관에 동의한 것으로 간주합니다. 회원은 약관 일부분만을 동의 또는 거부할 수 없습니다. 
- 회사는 제1항부터 제4항까지를 준수하였음에도 불구하고 회원이 약관 개정 사실을 알지 못함으로써 발생한 피해에 대해 회사의 고의 또는 중대한 과실이 없는 한 어떠한 책임도 지지 않습니다.

제4조(서비스의 제공) 
1. 회사는 다음 서비스를 제공합니다. 
- 점심식사, 스터디 모집, 팀 모집 등 생활 편의 서비스 
- 싸피 교육생 폐쇄형 커뮤니티 서비스 
- 기타 회사가 정하는 서비스 
2. 싸브리타임은 MatterMost 등을 통해 인증받은 이용자에 한해 커뮤니티를 이용할 수 있는 폐쇄형 서비스입니다. 싸브리타임은 인증을 받지 않은 이용자 및 불특정 다수의 일반 이용자는 커뮤니티를 포함한 대부분의 기능에 접근할 수 없습니다. 
3. 서비스 내에서 주고받은 메시지 등 회원이 전송한 콘텐츠는 회사 서버를 경유하면서 저장·보관될 수 있으며 이 정보는 회사만이 보유합니다. 회사는 관련 법령 및 이 약관과 개인정보처리방침에 따라 정보를 처리합니다. 
4. 회사는 운영상, 기술상의 필요에 따라 제공하고 있는 서비스를 변경할 수 있습니다. 
5. 회사는 이용자의 개인정보 및 서비스 이용 기록에 따라 서비스 이용에 차이를 둘 수 있습니다. 
6. 회사는 설비의 보수, 교체, 점검 또는 기간통신사업자의 서비스 중지, 인터넷 장애 등의 사유로 인해 일시적으로 서비스 제공이 어려울 경우, 통보 없이 일시적으로 서비스 제공을 중단할 수 있습니다. 
7. 회사는 천재지변, 전쟁, 경영 악화 등 불가항력적인 사유로 인해 서비스를 더 이상 제공하기 어려울 경우, 통보 없이 서비스 제공을 영구적으로 중단할 수 있습니다. 
8. 회사는 제4항부터 제7항까지 및 다음 내용으로 인해 발생한 피해에 대해 회사의 고의 또는 중대한 과실이 없는 한 어떠한 책임도 지지 않습니다. 
...
제5조(서비스 이용계약의 성립) 
1. 회사와 회원의 서비스 이용계약은 서비스를 이용하고자 하는 자(이하 "가입 신청자"라고 합니다)가 서비스 내부의 회원가입 양식에 따라 필요한 회원정보를 기입하고, 이 약관, 개인정보 수집 및 이용 동의, 커뮤니티 이용규칙 등에 명시적인 동의를 한 후, 신청한 회원가입 의사 표시(이하 "이용신청"이라 합니다)를 회사가 승낙함으로써 체결됩니다. 
...
3. 회원의 아이디, 비밀번호, 이메일 등 모든 개인정보의 관리책임은 본인에게 있으므로, 타인에게 양도 및 대여할 수 없으며 유출되지 않도록 관리해야 합니다. 만약 본인의 아이디 및 비밀번호를 타인이 사용하고 있음을 인지했을 경우, 즉시 문의 창구로 알려야 하고, 안내가 있는 경우 이에 따라야 합니다. 
...
제9조(저작권의 귀속) 
1. 회사는 저작권법에 따라 데이터베이스 제작자는 복제권 및 전송권을 포함한 데이터베이스 전부에 대한 권리를 가지고 있으며, 이는 법률에 따라 보호를 받는 대상입니다. 그러므로 이용자는 데이터베이스 제작자인 회사의 승인 없이 데이터베이스의 전부 또는 일부를 복제·배포·방송 또는 전송할 수 없습니다. 
...
- 성적 도의관념에 반하는 행위 
    ...
    - 방송미디어통신심의위원회의 정보통신에 관한 심의규정에 따른 심의기준의 성적 도의관념에 반하는 행위 
...
제15조(기타) 
1. 이 약관은 2024년 5월 30일에 개정되었습니다. 
2. 이 약관에도 불구하고, 회사와 이용자가 이 약관의 내용과 다르게 합의한 사항이 있는 경우에는 해당 내용을 우선으로 합니다.
3. 회사는 필요한 경우 약관의 하위 규정을 정할 수 있으며, 이 약관과 하위 규정이 상충하는 경우에는 이 약관의 내용이 우선합니다. 
4. 이 약관에서 정하지 아니한 사항과 이 약관의 해석에 관하여는 관련법 또는 관례에 따릅니다. 부칙(시행일) 이 약관은 2026년 2월 3일부터 시행됩니다."""