...
const MyPostsPage: React.FC = () => {
    ...
    const loadPosts = async () => {
        setLoading(true);
        try {
            // Using specialized endpoint for my posts
            const res = await myPageService.getMyPosts();
            setPosts(res);
        } catch (error) {
            console.error('내가 작성한 글 로딩 실패:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={{ minHeight: '100%', backgroundColor: 'var(--background)' }}>
            ...
            <div style={{
                ...
                <h1 style={{ fontSize: '22px', fontWeight: 'bold', margin: 0 }}>내가 작성한 글</h1>
            </div>
            ...
            <div className="container" style={{ padding: '40px 20px' }}>
                {loading && posts.length === 0 ? (
                    <div style={{ padding: '100px 0', textAlign: 'center', color: 'var(--on-surface-variant)' }}>
                        ...
                        <p style={{ marginTop: '16px' }}>글을 불러오는 중입니다...</p>
                    </div>
                ) : posts.length === 0 ? (
                    <div style={{
                        padding: '100px 0',
                        ...
                        <p style={{ fontSize: '18px' }}>작성한 글이 없습니다.</p>
                    </div>
                ) : (
                    <div style={{
                        maxWidth: '900px',
